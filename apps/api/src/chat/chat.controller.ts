...
export class ChatController {
  ...
  async createSession(@CurrentUser('id') userId: string, @Body() dto: CreateChatSessionDto) {
    this.logger.info('CHAT_CREATE_SESSION', `Creating new chat session`, { userId, title: dto.title });
    const session = await this.chatService.createSession(userId, dto.title);
    this.logger.info('CHAT_CREATE_SESSION', `Session created: ${session.id}`, { sessionId: session.id });
    return session;
  }

  @Get('sessions')
  getSessions(@CurrentUser('id') userId: string) {
    this.logger.debug('CHAT_LIST_SESSIONS', `Fetching sessions for user`, { userId });
    return this.chatService.getSessions(userId);
  }

  @Get('sessions/:id')
  getSession(@Param('id') id: string, @CurrentUser('id') userId: string) {
    this.logger.debug('CHAT_GET_SESSION', `Fetching session ${id}`, { sessionId: id });
    return this.chatService.getSession(id, userId);
  }

  @Delete('sessions/:id')
  async deleteSession(@Param('id') id: string, @CurrentUser('id') userId: string) {
    this.logger.info('CHAT_DELETE_SESSION', `Deleting session ${id}`, { sessionId: id });
    return this.chatService.deleteSession(id, userId);
  }

  @Get('sessions/:id/messages')
  getMessages(@Param('id') id: string, @CurrentUser('id') userId: string) {
    this.logger.debug('CHAT_GET_MESSAGES', `Fetching messages for session ${id}`, { sessionId: id });
  ...
  async sendMessage(
    ...
    this.logger.info('CHAT_SEND_MESSAGE', `Processing user message in session ${id}`, {
      sessionId: id,
    ...
    this.logger.debug('CHAT_SEND_MESSAGE', `User message saved`, { sessionId: id });
    ...
    let contextChunks: { content: string; documentTitle?: string; score?: number }[] = [];
    try {
      this.logger.info('CHAT_RAG', `Auto-searching across all READY documents`, { userId });
      ...
      if (docs.length > 0) {
        ...
        if (allChunks.length > 0) {
          this.logger.debug('CHAT_RAG', `Found ${allChunks.length} chunks across ${docs.length} documents`);

          // 3. Embed the user query
          const queryEmbedding = await this.localEmbedding.getEmbedding(dto.content);
          ...
          this.logger.info('CHAT_RAG', `Top chunk similarity: ${(contextChunks[0]?.score || 0).toFixed(4)}`, {
            topScore: contextChunks[0]?.score,
            documentsUsed: docs.map(d => d.title),
          });
        }
      } else {
        this.logger.info('CHAT_RAG', `No READY documents found for user`, { userId });
      }
    } catch (err: any) {
      this.logger.error('CHAT_RAG', `Auto-RAG failed: ${err.message}`, { error: err.message });
    }
    ...
    try {
      ...
      this.logger.info('CHAT_AI_CALL', `AI response received (${assistantReply.length} chars)`, {
